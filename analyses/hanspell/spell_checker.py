...
def _remove_tags(text):
    ...
    text = "<content>{}</content>".format(text)
    text2 = "<content>{}</content>".format(text2)
    ##
    if not PY3:
...
def check(text):
    ...
    # print(data["message"]["result"])
    checked_html = _remove_tags(html)
    result = {
        "result": True,
        "original": text,
        # "checked": _remove_tags(html),
        "checked": checked_html[0],
