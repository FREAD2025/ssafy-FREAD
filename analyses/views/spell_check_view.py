...
from drf_spectacular.utils import OpenApiResponse  # swagger
...
@extend_schema(
    ...
    responses={
        status.HTTP_200_OK: OpenApiResponse(
            response=SpellCheckResponseSerializer, description="맞춤법 검사 성공"
        ),
        status.HTTP_400_BAD_REQUEST: OpenApiResponse(
            description="잘못된 요청 데이터 (예: 텍스트 누락)"
        ),
        status.HTTP_503_SERVICE_UNAVAILABLE: OpenApiResponse(
            description="외부 맞춤법 검사 API 호출 실패 또는 서버 오류"
        ),
    },
)
@api_view(["POST"])
...
def spellcheck(request):  # API 설계도에 맞게 함수 이름 사용
    ...
    corrected_word_count_with_spaces = len(
        spell_check_result.get("corrected_text_plain", "")
    )
