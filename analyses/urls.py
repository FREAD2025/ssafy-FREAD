...
from .views import FreadAnalysisView
from . import views

urlpatterns = [
    path('', AnalysisView.as_view(), name='통합 분석 결과(GET)'),
    path('fread/', FreadAnalysisView.as_view(), name='fread 분석 요청(POST)'),
    path('fread/<int:analysis_id>/', FreadAnalysisView.as_view(), name='fread 분석 결과(GET)'),
