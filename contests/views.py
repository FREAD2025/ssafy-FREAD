from django.shortcuts import (
    get_object_or_404,
)  # 객체가 존재하지 않으면 404 에러를 자동으로 반환해주는 기능
from rest_framework import (
    status,
)  # HTTP 상태 코드를 쉽게 사용할 수 있도록 제공 (예: 200 OK, 404 Not Found)
from rest_framework.decorators import (
    api_view,
    permission_classes,
)  # API 뷰로 만들어주는 데코레이터와 권한 설정을 위한 데코레이터
...
)  # 목록을 여러 페이지로 나눠서 보여주는 기능

from .models import (
    Contest,
)  # 현재 앱의 Contest 모델을 가져옴 (데이터베이스 테이블과 연결된 클래스)
from .serializers import (
    ContestSerializer,
    SimpleContestSerializer,
)  # 현재 앱의 Serializer들을 가져옴 (모델 데이터를 JSON 등으로 변환해주는 도구)
...
@extend_schema(
    ...
    responses={
        ...
        status.HTTP_201_CREATED: OpenApiResponse(
            response=SimpleContestSerializer,
            description="공모전 등록 성공 (관리자 전용)",
        ),
        status.HTTP_400_BAD_REQUEST: OpenApiResponse(
            description="잘못된 요청 데이터 (등록 시)"
        ),
        status.HTTP_403_FORBIDDEN: OpenApiResponse(
            description="권한이 없는 사용자 (관리자만 등록 가능)"
        ),
        status.HTTP_401_UNAUTHORIZED: OpenApiResponse(
            description="인증되지 않은 사용자 (등록 시)"
        ),
    },
)
@api_view(["GET", "POST"])
def contest_list(request):
    ...
    if request.method == "GET":
        ...
        [
            ...
            ...
            {
                "id": 10,
        """
        serializer = SimpleContestSerializer(
            page, many=True, context={"request": request}
        )  # 나뉜 페이지의 공모전 데이터를 JSON 형태로 변환
        # 사용자가 로그인한 상태인지 확인하기 위해 request를 인자로 함께 넘겨줌 -> is_liked 필드의 값 결정
        return paginator.get_paginated_response(
            serializer.data
        )  # 페이지네이션된 응답과 함께 JSON 데이터를 반환
    ...
    # 공모전 등록 (관리자만 가능)
        ...
        if not request.user.is_staff:  # 요청한 사용자가 관리자가 아니라면
            ...
            )  # 403 Forbidden 에러 반환 (권한 없음)
        serializer = ContestSerializer(
            data=request.data, context={"request": request}
        )  # 요청으로 들어온 데이터로 Serializer 생성
        serializer.is_valid(
            raise_exception=True
        )  # 데이터 유효성 검사. 유효하지 않으면 에러 발생
        serializer.save()  # 유효한 데이터를 데이터베이스에 저장, 공모전 생성
        return Response(
            serializer.data, status=status.HTTP_201_CREATED
        )  # 성공적으로 생성되었음을 알리는 201 Created 응답과 함께 생성된 데이터 반환
...
@extend_schema(
    ...
    responses={
        status.HTTP_200_OK: OpenApiResponse(
            response=ContestSerializer, description="요청 성공"
        ),
        status.HTTP_204_NO_CONTENT: OpenApiResponse(
            description="공모전 삭제 성공 (DELETE)"
        ),
        status.HTTP_400_BAD_REQUEST: OpenApiResponse(
            description="잘못된 요청 데이터 (수정 시)"
        ),
        status.HTTP_401_UNAUTHORIZED: OpenApiResponse(
            description="인증되지 않은 사용자"
        ),
        status.HTTP_403_FORBIDDEN: OpenApiResponse(
            description="권한이 없는 사용자 (관리자만 수정/삭제 가능)"
        ),
        status.HTTP_404_NOT_FOUND: OpenApiResponse(
            description="해당 공모전이 존재하지 않음"
        ),
    },
)
...
def contest_detail(request, contest_id):
    ...
    )  # 주어진 ID의 공모전을 찾고, 없으면 404 에러 반환

    # 개별 공모전 상세 정보 조회
    if request.method == "GET":
        serializer = ContestSerializer(
            contest, context={"request": request}
        )  # 찾은 공모전 데이터를 JSON 형태로 변환
        return Response(serializer.data)  # JSON 데이터 반환

    # 아래 요청들은 관리자 권한이 필요함
    if not request.user.is_staff:  # 요청한 사용자가 관리자가 아니라면
        return Response(
            {"detail": "관리자 권한이 필요합니다."}, status=status.HTTP_403_FORBIDDEN
        )  # 403 Forbidden 에러 반환
    ...
    # 공모전 삭제
        ...
        )  # 성공적으로 삭제되었음을 알리는 204 No Content 응답 반환
...
def like_contest(request, contest_id):
    ...
    if request.method == "POST":
        ...
        contest.liked_users.add(
            user
        )  # 공모전의 'liked_users' (찜한 사용자들) 목록에 현재 사용자 추가
    ...
    # 찜 해제
        ...
        if (
            user not in contest.liked_users.all()
        ):  # 사용자가 이 공모전을 찜하지 않았다면
            return Response(
                {"detail": "찜하지 않은 공모전입니다."},
                status=status.HTTP_400_BAD_REQUEST,
            )  # 400 Bad Request 에러 반환
        contest.liked_users.remove(
            user
        )  # 공모전의 'liked_users' 목록에서 현재 사용자 제거
        likes_count = contest.liked_users.count()  # 좋아요 수 조회
        return Response(
            {
                "status": "unliked",
