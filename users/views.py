...
@extend_schema(  # swagger에서 표시
    ...
    responses={
        status.HTTP_201_CREATED: OpenApiResponse(
            response=SignupSerializer, description="회원 가입 성공 및 사용자 정보 반환"
        ),
...
@extend_schema(
    ...
    responses={
        status.HTTP_200_OK: OpenApiResponse(
            response=SocialExtraInfoSerializer,
            description="추가 정보 입력 성공 및 업데이트된 사용자 정보 반환",
        ),
        status.HTTP_400_BAD_REQUEST: OpenApiResponse(
            description="잘못된 요청 데이터 또는 이미 추가 정보를 입력한 경우"
        ),
...
def login(request):
    ...
    if serializer.is_valid(raise_exception=True):
        ...
        )  # Serializer의 validate 메서드에서 반환된 User 객체
        auth_login(request, user)
        return Response(
            {
                "message": "로그인 성공",
...
@extend_schema(
    ...
    description="이메일 주소와 일치하는 사용자의 아이디를 DB에서 찾아 바로 반환합니다.",
    request=FindIdSerializer,
    responses={
        status.HTTP_200_OK: OpenApiResponse(description="아이디 찾기 성공"),
        status.HTTP_400_BAD_REQUEST: OpenApiResponse(
            description="유효하지 않은 요청 데이터"
        ),
        status.HTTP_404_NOT_FOUND: OpenApiResponse(
            description="해당 이메일 주소로 등록된 사용자 없음"
        ),
    },
)
@api_view(["POST"])
...
@extend_schema(
    ...
    responses={
        status.HTTP_200_OK: OpenApiResponse(
            description="임시 비밀번호 발송 성공, 이메일 발송됨"
        ),
        status.HTTP_400_BAD_REQUEST: OpenApiResponse(
            description="유효하지 않은 요청 데이터"
        ),
...
def reset_password(request):
    ...
    try:
        ...
        try:
            ...
            return Response(
                {"detail": "이메일 발송 중 오류가 발생했습니다."}, status=500
            )
...
@extend_schema(
    ...
    responses={
        ...
        status.HTTP_400_BAD_REQUEST: OpenApiResponse(
            description="잘못된 요청 데이터 (비밀번호 불일치, 형식 오류 등)"
        ),
        status.HTTP_401_UNAUTHORIZED: OpenApiResponse(
...
@extend_schema(
    ...
    responses={
        status.HTTP_200_OK: OpenApiResponse(
            response=MyPageSerializer, description="마이페이지 정보 조회 성공"
        ),
        status.HTTP_401_UNAUTHORIZED: OpenApiResponse(
            description="인증되지 않은 사용자"
        ),
...
def liked_contests(request):
    ...
    queryset = request.user.liked_contests.all().order_by(
        "end_date"
    )  # 정렬 기준 확인 필요
    # queryset 예시
    """
