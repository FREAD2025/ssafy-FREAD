...
class SignupSerializer(serializers.ModelSerializer):
    ...
    # 예. 너무 짧은 비밀번호, 너무 흔한 비밀번호, 숫자만 이루어진 비밀번호, 사용자 이름과 유사한 비밀번호는 거절됨
    ...
    def validate_username(self, value):
        if User.objects.filter(
            username=value
        ).exists():  # 이미 해당 value와 동일한 username이 있다면
    ...
    def validate_email(self, value):
        if User.objects.filter(
            email=value
        ).exists():  # 이미 해당 value와 동일한 email이 있다면
    ...
    def create(self, validated_data):
        ...
        user = User.objects.create_user(
            **validated_data
        )  # 새로운 인스턴스 생성 # 비밀번호는 해싱 후 저장됨
        user.genres.set(
            genres_data
        )  # .set(genres_data) 메서드: mtm 필드에 주어진 리스트의 ID에 해당하는 Genre 객체들을 연결함
...
class FindIdSerializer(serializers.Serializer):
    ...
    email = serializers.EmailField(
        required=True, help_text="가입 시 사용한 이메일 주소를 입력하세요"
    )
...
class PasswordChangeSerializer(serializers.Serializer):
    ...
    def save(self, **kwargs):
        ...
        user.set_password(
            self.validated_data["new_password1"]
        )  # 새 비밀번호를 암호화하여 저장
...
class MyPageSerializer(serializers.Serializer):
    ...
    def get_profile(self, obj):
        # obj 는 user 인스턴스
        return ReadOnlyProfileSerializer(obj, context=self.context).data
    ...
    def get_recent_liked_contests(self, obj):
        ...
        queryset = obj.liked_contests.all().order_by("end_date")[
            :8
        ]  # 마감일이 가까운 순으로 공모전 정렬
